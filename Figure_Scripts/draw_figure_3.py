...
#%%
###################################
path_1 = "./Data/Gene_Enrichment/gene_enrich_weight_count_suicidal_event_squared.fdr_05.methdiff_5.count_90perc.region.annotation.Annotatr.KEGG.csv"
path_2 = "./Data/Gene_Enrichment/gene_enrich_weight_count_suicidal_event_squared.fdr_05.methdiff_5.count_90perc.region.annotation.Annotatr.GOCC.csv"

name_1 = "KEGG"
name_2 = "GO (Cellular Component)"
...
table_pathway2 = pd.read_csv(path_2)

table_pathway1["Pathway_id"] = table_pathway1["Pathway"].apply(lambda x : x.split()[0])
table_pathway2["Pathway_id"] = table_pathway2["Pathway"].apply(lambda x : x.split()[0])
table_pathway1["Pathway_name"] = table_pathway1["Pathway"].apply(lambda x : ' '.join(x.split()[1:]))
table_pathway2["Pathway_name"] = table_pathway2["Pathway"].apply(lambda x : ' '.join(x.split()[1:]))

if Upper_first:
    table_pathway1["Pathway_name"] = table_pathway1["Pathway_name"].apply(lambda x : x[0].upper() + x[1:])
    table_pathway2["Pathway_name"] = table_pathway2["Pathway_name"].apply(lambda x : x[0].upper() + x[1:])
...
table_pathway2["Pathway_show"] = table_pathway2[cols_show_pathway].apply(lambda row : ':'.join(row.to_list()), axis = 1)
...
if adjust_showing_pathway_length:
    table_pathway1["Pathway_show"] = table_pathway1["Pathway_show"].apply(lambda val : adjust_phrase_length(val, len_showing_adjust_kegg))
    table_pathway2["Pathway_show"] = table_pathway2["Pathway_show"].apply(lambda val : adjust_phrase_length(val, len_showing_adjust_go))
...
table_pathway2_filtered = filter_table_with_fdr_fe(table_pathway2, n_max_show)

hue_min = min(table_pathway1_filtered[hue_factor].to_list() + table_pathway2_filtered[hue_factor].to_list())
hue_max = max(table_pathway1_filtered[hue_factor].to_list() + table_pathway2_filtered[hue_factor].to_list())
size_min = min(table_pathway1_filtered[size_factor].to_list() + table_pathway2_filtered[size_factor].to_list())
size_max = max(table_pathway1_filtered[size_factor].to_list() + table_pathway2_filtered[size_factor].to_list())
...
gs2 = gsfig[0:82, 70:100]
ax2 = fig.add_subplot(gs2)

gs3 = gsfig[91:109, 7:44] 
ax3 = fig.add_subplot(gs3)

gs4 = gsfig[98:101, 50:85]
ax4 = fig.add_subplot(gs4)

gs5 = gsfig[90:110, 0:93]
...
sns.scatterplot(data = table_pathway2_filtered,
                ...
                legend = False,
                zorder = 3)
tick_colors = list(map({1:color_label_significant, 0:color_label_nonsignificant}.__getitem__, table_pathway2_filtered["FDR_sig"]))
for ticklabel, tickcolor in zip(ax2.get_yticklabels(), tick_colors):
    ticklabel.set_color(tickcolor)
    if tickcolor == color_label_significant:
        ticklabel.set_fontweight("bold")
    ticklabel.set_fontsize(plt.rcParams['font.size'] + ytick_extra_size)
ax2.grid(zorder = 1, linewidth = 0.5)
ax2.set_ylabel("")
ax2.set_title(name_2, fontsize = plt.rcParams["font.size"] + 1)
ax2.set_xlim(1, 9)
ax2.set_xticks(range(2, 9, 2))
ax2.text(-1.1, 1.02, "B", transform = ax2.transAxes, size = fontsize_label, weight = "bold")
...
plt.savefig("./Figures/Figure_3.pdf", dpi = 650, bbox_inches='tight')
plt.savefig("./Figures/Figure_3.png", dpi = 650, bbox_inches='tight')
plt.show()
 # %%
